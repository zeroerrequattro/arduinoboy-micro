void modeProgrammer()
{
  while(sysexProgrammingConnected || sysexProgrammingMode) {
    ...
    if (Serial1.available()) checkForProgrammerSysex(Serial1.read());
...
{
  Serial1.write(0xF0);
  Serial1.write(sysexManufacturerId);
  Serial1.write(0x40);
  sendMemory();
  Serial1.write(0xF7);
}
...
void programmerSendConnectRequest()
{
  if(millis() > (sysexProgrammerLastSent+sysexProgrammerCallTime)) {
    ...
    Serial1.write(sysexManufacturerId);
    Serial1.write(0x7F);
    Serial1.write(defaultMemoryMap[MEM_VERSION_FIRST]);
    Serial1.write(defaultMemoryMap[MEM_VERSION_SECOND]);
    Serial1.write(0xF7);
