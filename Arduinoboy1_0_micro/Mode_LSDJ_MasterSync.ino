...
void modeLSDJMasterSyncSetup()
{
  ...
  DDRF  = B00000000; //Set analog in pins as inputs
  countSyncTime=0;
  blinkMaxCount=1000;
  modeLSDJMasterSync();
}
...
void modeLSDJMasterSync()
{
  while(1){
    if (Serial1.available()) {                  //If serial data was send to midi input
      ...
      if(!checkForProgrammerSysex(incomingMidiByte) && !usbMode) Serial1.write(incomingMidiByte);        //Send it to the midi output
    }
    ...
    if(readgbClockLine) {                          //If Gb's Clock is On
      while(readgbClockLine) {                     //Loop untill its off
        ...
        bit = (PINF & 0x04)>>2;                   //Read the serial input for song position
        checkActions();
      }
...
void sendMidiClockSlaveFromLSDJ()
{
  if(!countGbClockTicks) {      //If we hit 8 bits
    if(!sequencerStarted) {         //If the sequencer hasnt started
      Serial1.write((0x90+memory[MEM_LSDJMASTER_MIDI_CH])); //Send the midi channel byte
      Serial1.write(readGbSerialIn);                //Send the row value as a note
      Serial1.write(0x7F);                          //Send a velocity 127
      
      Serial1.write(0xFA);     //send MIDI transport start message 
      sequencerStart();             //call the global sequencer start function
    }
    Serial1.write(0xF8);       //Send the MIDI Clock Tick
